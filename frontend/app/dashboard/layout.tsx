  ...
  if (status === 'loading' || !mounted) {
    return (
      <div className="fixed inset-0 bg-black flex items-center justify-center z-[100]">
        <div
          className="w-3 h-3 bg-[#C9A84C] rounded-full"
    ...
    <div className="min-h-screen bg-black flex selection:bg-[#C9A84C] selection:text-black text-[#ededed]">
      ...
      <aside className="fixed left-0 top-0 bottom-0 w-[240px] bg-[#0a0a0a] border-r border-[#2a2a2a] z-50 flex flex-col">
        <div className="p-8">
          <Link
            href="/"
            className="text-xl font-bold text-[#C9A84C] tracking-tighter hover:opacity-80 transition-opacity block"
          >
            CodeHalcon 🦅
          </Link>
        </div>
      ...
      <main className="ml-[240px] flex-1 relative min-h-screen flex flex-col">
        {/* Background layers — static, paint once */}
        ...
        {/*
          Top Bar — removed backdrop-blur-md (triggers repaint on every scroll).
          Replaced with a solid semi-transparent bg which composites for free.
        */}
        <header className="h-[60px] border-b border-[#2a2a2a] flex items-center justify-between px-8 bg-[#050505]/95 sticky top-0 z-40">
